...
const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    // THIS IS THE FIX
    const aiClient = new genAI.GoogleGenerativeAI(process.env.GEMINI_API_KEY);
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: error.message || 'An error occurred while generating ASCII art.' })
    };
  }
};
